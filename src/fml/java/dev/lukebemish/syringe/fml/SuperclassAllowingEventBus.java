...
final class SuperclassAllowingEventBus implements IEventBus {
    ...
    private static MethodHandles.Lookup getGameLayerLookup() {
        ...
        try {
            ...
            return (MethodHandles.Lookup) lookupMethod.invoke();
        } catch (Throwable e) {
    ...
    SuperclassAllowingEventBus(IEventBus delegate) {
    ...
    private static Function<?, ?> makeWrapper(Class<?> classToWrap) {
        ...
        var name = ATTACHMENT_TARGET_NAME.replace('.', '/') + "$$SyringeEventBusWrapper";
